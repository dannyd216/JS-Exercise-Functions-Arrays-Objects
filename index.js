 ...
 * 
 * 
 *  "THIS FUNCT TAKES AS ITS ONLY ARG AN OBJECT" => DUE TO AN OBJECT BEING PASSED {ID:1, NAME:LEIA, EMAIL: ....}
 * WE NEED TO ACCESS THE NAME KEY:VALUE PAIR 
 * 
 * 
 */
...
*/
function makeSmartPerson(whoSmart) {
  /* code here */
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(invArray, idNum) {
  /* code here */

  return `This is a ${invArray[idNum - 1].car_make} ${invArray[idNum - 1].car_model}`;
...
*/
function sortCarInventory(invenArray) {
  /* code here */

  return invenArray.sort((a,b) => a.car_model < b.car_model ? -1 : a.car_model ? 1 : 0);
}
...
function getModelYears(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    newYearArray.push(array[i].car_year);
...
*/
function getOlderCars(carsInv, numb) {
  /* code here */

  let oldCarsArray = [];

  for ( let i = 0; i < carsInv.length; i++) {

    if (carsInv[i].car_year <= numb) 
    {
      oldCarsArray.push(carsInv[i]);
    }
  }
